...
#ifdef DEBUG
  printf("wrote NS record, return code: %i\n", ret2);
#endif
...
#ifdef DEBUG
  printf("wrote SOA record, return code: %i\n", ret2);
#endif
    if (!ret2) { outbuf[7]++; }
  }
...
#ifdef DEBUG
  printf("wrote A record, return code: %i\n", ret);
#endif
      if (!ret) {
        n++;
        outbuf[7]++;
...
int dnsserver(dns_opt_t *opt) {
  ...
  if (senderSocket == -1) {
    perror("\nERROR: cannot create udp socket: senderSocket");
    exit(-3);
  }
  ...
  if (listenSocket == -1) {
    ...
    if ((listenSocket=socket(AF_INET6, SOCK_DGRAM, IPPROTO_UDP))==-1) {
      listenSocket = -1;
      perror("\nERROR: cannot create udp socket: listenSocket");
      exit(-1);
    }
    ...
    {
      close(listenSocket);
      perror("\nERROR: cannot create udp socket: replySocket");
      return -1;
    }
    int sockopt = 1;
    ...
    if (bind(listenSocket, (struct sockaddr*)&si_me, sizeof(si_me))==-1) {
      perror("\nERROR: cannot bind listenSocket on specified port");
      exit(-2);
    }
  }
  ...
  printf("DNS: Request %llu from %i.%i.%i.%i.%i.%i.%i.%i:%i of %i bytes\n", (unsigned long long)(opt->nRequests), addr[0], addr[1], addr[2], addr[3], addr[4], addr[5], addr[6], addr[7], ntohs(si_other.sin6_port), (int)insize);
#endif
    if (insize <= 0)
      continue;
